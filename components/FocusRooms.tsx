...
import { Users, Wifi, LogIn, User as UserIcon, Atom, Dna, GraduationCap, ArrowLeft, Loader2 } from 'lucide-react';
...
const FocusRooms: React.FC<FocusRoomsProps> = ({ 
    user, 
    onLogin,
    timerState,
    timeLeft,
  ...
  useEffect(() => {
    ...
    const unsubscribes = roomKeys.map(roomName => {
        const roomRef = ref(rtdb, `rooms/${roomName}/presence`);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomPresenceRef, (snapshot) => {
        ...
        console.error("Firebase RTDB error in Focus Rooms:", error);
        setIsLoading(false);
  ...
  if (!user) {
      return (
          <div className="flex flex-col items-center justify-center h-full min-h-[500px] text-center p-6 animate-in fade-in duration-500">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Join a Focus Room</h2>
        ...
        {view === 'lobby' ? (
            <>
                <div>
                    <h2 className="text-3xl font-bold text-theme-text tracking-tight flex items-center gap-3">
                        <Users className="text-indigo-400" /> Focus Rooms
                    </h2>
            ...
            <>
                ...
                <Card className="p-6">
                    ...
                    {isLoading ? (
                        <div className="text-center py-10 text-slate-400 text-sm font-medium flex items-center justify-center gap-2">
